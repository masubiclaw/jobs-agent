        ...
        for page_attempt in range(max_page_attempts):
            ...
            if is_single_page:
                ...
                page_feedback = (
                    ...
                    f"\n"
                    f"PRIORITIZATION PROCESS (follow this order):\n"
                    f"1. SCORE every experience role 0-10 by relevance to the target job:\n"
                    f"   - Shared skills/technologies with the job description\n"
                    f"   - Matching domain, seniority, and scope\n"
                    f"   - Impact and measurable outcomes\n"
                    f"2. SCORE every accomplishment bullet within each role 0-10:\n"
                    f"   - Directly demonstrates a required job skill\n"
                    f"   - Has quantified impact (metrics, $, %)\n"
                    f"   - Uses keywords from the job description\n"
                    f"3. RANK all roles and bullets by score\n"
                    f"4. TRIM from the LOWEST-scoring items first:\n"
                    f"   - Drop bullets scoring <5 before dropping whole roles\n"
                    f"   - Drop whole roles only if removing their bullets isn't enough\n"
                    f"   - NEVER drop the current role (top of resume)\n"
                    f"   - Prefer dropping older roles with low relevance over recent ones with high relevance\n"
                    f"5. KEEP the highest-scoring 4-5 roles with their top 2-4 bullets\n"
                    f"\n"
                    f"HARD LIMITS after trimming:\n"
                    f"- Skills section: 10-12 most relevant, comma-separated\n"
                    f"- Summary: 2 sentences maximum\n"
                    f"- Total resume: 400-500 words\n"
                    f"- Each bullet: 12-18 words\n"
                    f"\n"
                    f"Explain in a comment at the top which role(s) or bullet(s) you dropped and why."
                )
