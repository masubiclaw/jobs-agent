#!/usr/bin/env python3
"""
Show top job matches with links.

Usage:
    python scripts/show_top_matches.py              # Show matches >= 70%
    python scripts/show_top_matches.py --min 50     # Show matches >= 50%
    python scripts/show_top_matches.py --limit 10   # Limit to 10 results
    python scripts/show_top_matches.py --show-excluded  # Include excluded companies
"""

import argparse
import sys
from pathlib import Path

sys.path.insert(0, str(Path(__file__).parent.parent))

from job_agent_coordinator.tools.job_cache import get_cache
from job_agent_coordinator.tools.profile_store import get_store


def main():
    parser = argparse.ArgumentParser(description="Show top job matches with links")
    parser.add_argument("--min", type=int, default=70, help="Minimum score (default: 70)")
    parser.add_argument("--limit", type=int, default=100, help="Max results (default: 100)")
    parser.add_argument("--show-excluded", action="store_true", help="Include excluded companies in output")
    args = parser.parse_args()

    cache = get_cache()
    matches = cache.list_matches(min_score=args.min, limit=args.limit)
    jobs = {j['id']: j for j in cache.list_all(limit=10000)}
    
    # Load exclusions from profile for display and filtering
    context = get_store().get_search_context()
    excluded = [c.lower() for c in context.get("excluded_companies", [])]

    print("=" * 80)
    print(f"TOP MATCHES (score >= {args.min}%)")
    print("=" * 80)
    if excluded and not args.show_excluded:
        print(f"Excluding: {', '.join(context.get('excluded_companies', []))}")
    print(f"Found: {len(matches)} matches\n")

    display_count = 0
    for m in matches:
        job_id = m.get("job_id", "")
        job = jobs.get(job_id, {})
        
        company = job.get("company", "Unknown")
        
        # Double-check exclusion (list_matches already filters, but be safe)
        if not args.show_excluded and excluded:
            if any(exc in company.lower() for exc in excluded):
                continue
        
        display_count += 1
        score = m.get("combined_score", m.get("keyword_score", m.get("match_score", 0)))
        kw = m.get("keyword_score", 0)
        llm = m.get("llm_score")
        
        title = job.get("title", "Unknown")
        location = job.get("location", "")
        url = job.get("url", "")
        
        llm_str = f" | llm={llm}%" if llm is not None else ""
        
        print(f"{display_count}. [{score}%] {title}")
        print(f"   Company: {company}")
        if location:
            print(f"   Location: {location}")
        print(f"   Scores: kw={kw}%{llm_str}")
        if url:
            print(f"   Link: {url}")
        else:
            print(f"   Link: (not available)")
        print()

    if display_count == 0:
        print(f"No matches found with score >= {args.min}%")


if __name__ == "__main__":
    main()
